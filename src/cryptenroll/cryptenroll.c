...
static uint32_t arg_tpm2_srk = 0;
static char *arg_tpm2_srk_public_key = NULL;
...
STATIC_DESTRUCTOR_REGISTER(arg_tpm2_srk_public_key, freep);
...
static int help(void) {
        ...
        printf("%s [OPTIONS...] BLOCK-DEVICE\n"
               ...
               "     --tpm2-srk=HANDLE\n"
               "                       Specify the handle containing the storage root key\n"
               "     --tpm2-srk-public-key=PATH\n"
               "                       Specify the storage root key PEM public key to use\n"
...
static int parse_argv(int argc, char *argv[]) {
        ...
        static const struct option options[] = {
                ...
                { "tpm2-srk",                     required_argument, NULL, ARG_TPM2_SRK              },
                { "tpm2-srk-public-key",          required_argument, NULL, ARG_TPM2_SRK_PUBLIC_KEY   },
        ...
        while ((c = getopt_long(argc, argv, "h", options, NULL)) >= 0) {
                ...
                case ARG_TPM2_SRK:
                        if (strcaseeq(optarg, "SRK")) {
                                arg_tpm2_srk = TPM2_PERSISTENT_SRK;
                                break;
                        }

                        r = tpm2_parse_persistent_handle(optarg, &arg_tpm2_srk);
                        if (r < 0)
                                return r;

                        break;

                case ARG_TPM2_SRK_PUBLIC_KEY:
                        r = parse_path_argument(optarg, /* suppress_root= */ false, &arg_tpm2_srk_public_key);
                        if (r < 0)
                                return r;
...
static int run(int argc, char *argv[]) {
        ...
        case ENROLL_TPM2:
                slot = enroll_tpm2(cd, vk, vks, arg_tpm2_device, arg_tpm2_srk, arg_tpm2_srk_public_key, arg_tpm2_pcr_mask, arg_tpm2_public_key, arg_tpm2_public_key_pcr_mask, arg_tpm2_signature, arg_tpm2_pin);
                break;
