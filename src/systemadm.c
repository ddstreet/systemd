/* systemadm.c generated by valac 0.11.2, the Vala compiler
...
MainWindow* main_window_construct (GType object_type, GError** error) {
	...
	GtkButton* _tmp129_;
	DBusBusType _tmp130_ = 0;
	DBusGConnection* _tmp131_ = NULL;
	DBusGConnection* _tmp132_;
	DBusGConnection* _tmp133_;
	DBusGProxy* _tmp134_ = NULL;
	Manager* _tmp135_;
	...
	if (user) {
		_tmp130_ = DBUS_BUS_SESSION;
	} else {
		_tmp130_ = DBUS_BUS_SYSTEM;
	}
	_tmp131_ = dbus_g_bus_get (_tmp130_, &_inner_error_);
	_tmp132_ = _tmp131_;
	...
	self->priv->bus = _tmp133_;
	_tmp134_ = dbus_g_proxy_new_for_name (self->priv->bus, "org.freedesktop.systemd1", "/org/freedesktop/systemd1", "org.freedesktop.systemd1.Manager");
	_tmp135_ = manager_dbus_proxy_new (self->priv->bus, "org.freedesktop.systemd1", "/org/freedesktop/systemd1");
	_g_object_unref0 (self->priv->manager);
	self->priv->manager = _tmp135_;
...
void main_window_populate_unit_model (MainWindow* self, GError** error) {
	...
	}
	{
		...
		for (i_it = 0; i_it < list_length1; i_it = i_it + 1) {
			ManagerUnitInfo _tmp3_;
			ManagerUnitInfo _tmp4_ = {0};
			ManagerUnitInfo i;
			_tmp3_ = (manager_unit_info_copy (&i_collection[i_it], &_tmp4_), _tmp4_);
			i = _tmp3_;
			{
				GtkTreeIter iter = {0};
				...
				gchar* _tmp8_;
				_tmp5_ = dbus_g_proxy_new_for_name (self->priv->bus, "org.freedesktop.systemd1", (const gchar*) i.unit_path, "org.freedesktop.DBus.Properties");
				p = properties_dbus_proxy_new (self->priv->bus, "org.freedesktop.systemd1", (const gchar*) i.unit_path);
				g_signal_connect_object (p, "properties-changed", (GCallback) _main_window_on_unit_changed_properties_properties_changed, self, 0);
				_tmp6_ = dbus_g_proxy_new_for_name (self->priv->bus, "org.freedesktop.systemd1", (const gchar*) i.unit_path, "org.freedesktop.systemd1.Unit");
				u = unit_dbus_proxy_new (self->priv->bus, "org.freedesktop.systemd1", (const gchar*) i.unit_path);
				gtk_list_store_append (self->priv->unit_model, &_tmp7_);
				iter = _tmp7_;
				_tmp8_ = NULL;
				if (g_strcmp0 (i.job_type, "") != 0) {
					gchar* _tmp9_ = NULL;
					gchar* _tmp10_;
					_tmp9_ = g_strdup_printf ("→ %s", i.job_type);
					_tmp10_ = _tmp9_;
					_g_free0 (_tmp8_);
					_tmp8_ = _tmp10_;
				} else {
				...
				gtk_list_store_set (self->priv->unit_model, &iter, 0, i.id, 1, i.description, 2, i.load_state, 3, i.active_state, 4, i.sub_state, 5, _tmp8_, 6, u, -1);
				_g_free0 (_tmp8_);
				_g_object_unref0 (u);
				_g_object_unref0 (p);
...
void main_window_populate_job_model (MainWindow* self, GError** error) {
	...
	}
	{
		...
		i_collection_length1 = list_length1;
		for (i_it = 0; i_it < list_length1; i_it = i_it + 1) {
			ManagerJobInfo _tmp3_;
			ManagerJobInfo _tmp4_ = {0};
			ManagerJobInfo i;
			_tmp3_ = (manager_job_info_copy (&i_collection[i_it], &_tmp4_), _tmp4_);
			i = _tmp3_;
			{
				GtkTreeIter iter = {0};
				DBusGProxy* _tmp5_ = NULL;
				...
				gchar* _tmp10_ = NULL;
				gchar* _tmp11_;
				_tmp5_ = dbus_g_proxy_new_for_name (self->priv->bus, "org.freedesktop.systemd1", (const gchar*) i.job_path, "org.freedesktop.DBus.Properties");
				p = properties_dbus_proxy_new (self->priv->bus, "org.freedesktop.systemd1", (const gchar*) i.job_path);
				g_signal_connect_object (p, "properties-changed", (GCallback) _main_window_on_job_changed_properties_properties_changed, self, 0);
				_tmp6_ = dbus_g_proxy_new_for_name (self->priv->bus, "org.freedesktop.systemd1", (const gchar*) i.job_path, "org.freedesktop.systemd1.Job");
				j = job_dbus_proxy_new (self->priv->bus, "org.freedesktop.systemd1", (const gchar*) i.job_path);
				gtk_list_store_append (self->priv->job_model, &_tmp7_);
				iter = _tmp7_;
				_tmp8_ = g_strdup_printf ("%u", (guint) i.id);
				_tmp9_ = _tmp8_;
				_tmp10_ = g_strdup_printf ("→ %s", i.type);
				_tmp11_ = _tmp10_;
				gtk_list_store_set (self->priv->job_model, &iter, 0, _tmp9_, 1, i.name, 2, _tmp11_, 3, i.state, 4, j, 5, i.id, -1);
				_g_free0 (_tmp11_);
				_g_free0 (_tmp9_);
				_g_object_unref0 (j);
				_g_object_unref0 (p);
...
void main_window_show_unit (MainWindow* self, Unit* unit) {
	...
	requires = _tmp12_;
	_tmp13_ = unit_get_requires_overridable (unit, &_tmp14_);
	_tmp15_ = _tmp13_;
	requires_overridable_length1 = _tmp14_;
	_requires_overridable_size_ = requires_overridable_length1;
	requires_overridable = _tmp15_;
	_tmp16_ = unit_get_requisite (unit, &_tmp17_);
	_tmp18_ = _tmp16_;
	requisite_length1 = _tmp17_;
	_requisite_size_ = requisite_length1;
	requisite = _tmp18_;
	_tmp19_ = unit_get_requisite_overridable (unit, &_tmp20_);
	_tmp21_ = _tmp19_;
	requisite_overridable_length1 = _tmp20_;
	_requisite_overridable_size_ = requisite_overridable_length1;
	requisite_overridable = _tmp21_;
	_tmp22_ = unit_get_wants (unit, &_tmp23_);
	_tmp24_ = _tmp22_;
	wants_length1 = _tmp23_;
	_wants_size_ = wants_length1;
	wants = _tmp24_;
	_tmp25_ = unit_get_required_by (unit, &_tmp26_);
	_tmp27_ = _tmp25_;
	required_by_length1 = _tmp26_;
	_required_by_size_ = required_by_length1;
	required_by = _tmp27_;
	_tmp28_ = unit_get_required_by_overridable (unit, &_tmp29_);
	_tmp30_ = _tmp28_;
	required_by_overridable_length1 = _tmp29_;
	_required_by_overridable_size_ = required_by_overridable_length1;
	required_by_overridable = _tmp30_;
	_tmp31_ = unit_get_wanted_by (unit, &_tmp32_);
	_tmp33_ = _tmp31_;
	wanted_by_length1 = _tmp32_;
	_wanted_by_size_ = wanted_by_length1;
	wanted_by = _tmp33_;
	_tmp34_ = unit_get_conflicts (unit, &_tmp35_);
	_tmp36_ = _tmp34_;
	conflicts_length1 = _tmp35_;
	_conflicts_size_ = conflicts_length1;
	conflicts = _tmp36_;
	_tmp37_ = unit_get_before (unit, &_tmp38_);
	_tmp39_ = _tmp37_;
	before_length1 = _tmp38_;
	_before_size_ = before_length1;
	before = _tmp39_;
	_tmp40_ = unit_get_after (unit, &_tmp41_);
	_tmp42_ = _tmp40_;
	after_length1 = _tmp41_;
	_after_size_ = after_length1;
	after = _tmp42_;
	_tmp43_ = main_window_make_dependency_string (self, NULL, "requires", requires, requires_length1);
	_tmp44_ = _tmp43_;
	_tmp45_ = main_window_make_dependency_string (self, _tmp44_, "overridable requires", requires_overridable, requires_overridable_length1);
	_tmp46_ = _tmp45_;
	_tmp47_ = main_window_make_dependency_string (self, _tmp46_, "requisite", requisite, requisite_length1);
	_tmp48_ = _tmp47_;
	_tmp49_ = main_window_make_dependency_string (self, _tmp48_, "overridable requisite", requisite_overridable, requisite_overridable_length1);
	_tmp50_ = _tmp49_;
	_tmp51_ = main_window_make_dependency_string (self, _tmp50_, "wants", wants, wants_length1);
	_tmp52_ = _tmp51_;
	_tmp53_ = main_window_make_dependency_string (self, _tmp52_, "conflicts", conflicts, conflicts_length1);
	_tmp54_ = _tmp53_;
	_tmp55_ = main_window_make_dependency_string (self, _tmp54_, "required by", required_by, required_by_length1);
	_tmp56_ = _tmp55_;
	_tmp57_ = main_window_make_dependency_string (self, _tmp56_, "overridable required by", required_by_overridable, required_by_overridable_length1);
	_tmp58_ = _tmp57_;
	_tmp59_ = main_window_make_dependency_string (self, _tmp58_, "wanted by", wanted_by, wanted_by_length1);
	_tmp60_ = _tmp59_;
	_tmp61_ = main_window_make_dependency_string (self, _tmp60_, "after", after, after_length1);
	_tmp62_ = _tmp61_;
	_tmp63_ = main_window_make_dependency_string (self, _tmp62_, "before", before, before_length1);
	_tmp64_ = _tmp63_;
	right_label_set_markup_or_na (self->priv->unit_dependency_label, _tmp64_);
	_g_free0 (_tmp64_);
	_g_free0 (_tmp62_);
	_g_free0 (_tmp60_);
	_g_free0 (_tmp58_);
	...
	_g_free0 (_tmp46_);
	_g_free0 (_tmp44_);
	_tmp65_ = unit_get_description (unit);
	_tmp66_ = _tmp65_;
	right_label_set_text_or_na (self->priv->unit_description_label, _tmp66_);
	_g_free0 (_tmp66_);
	_tmp67_ = unit_get_load_state (unit);
	_tmp68_ = _tmp67_;
	right_label_set_text_or_na (self->priv->unit_load_state_label, _tmp68_);
	_g_free0 (_tmp68_);
	_tmp69_ = unit_get_active_state (unit);
	_tmp70_ = _tmp69_;
	right_label_set_text_or_na (self->priv->unit_active_state_label, _tmp70_);
	_g_free0 (_tmp70_);
	_tmp71_ = unit_get_sub_state (unit);
	_tmp72_ = _tmp71_;
	right_label_set_text_or_na (self->priv->unit_sub_state_label, _tmp72_);
	_g_free0 (_tmp72_);
	_tmp73_ = unit_get_fragment_path (unit);
	fp = _tmp73_;
	if (g_strcmp0 (fp, "") != 0) {
		gchar* _tmp74_;
		gchar* _tmp75_;
		gchar* _tmp76_;
		gchar* _tmp77_;
		_tmp74_ = g_strconcat ("<a href=\"file://", fp, NULL);
		_tmp75_ = g_strconcat (_tmp74_, "\">", NULL);
		_tmp76_ = g_strconcat (_tmp75_, fp, NULL);
		_tmp77_ = g_strconcat (_tmp76_, "</a>", NULL);
		right_label_set_markup_or_na (self->priv->unit_fragment_path_label, _tmp77_);
		_g_free0 (_tmp77_);
		_g_free0 (_tmp76_);
		_g_free0 (_tmp75_);
		_g_free0 (_tmp74_);
	...
	_tmp78_ = unit_get_active_enter_timestamp (unit);
	t = _tmp78_;
	if (t > 0) {
		struct tm _tmp79_ = {0};
		struct tm _tmp80_ = {0};
		...
		gchar* _tmp81_ = NULL;
		gchar* _tmp82_;
		_tmp80_ = (g_time_local ((time_t) (t / 1000000), &_tmp79_), _tmp79_);
		timestamp = _tmp80_;
		_tmp81_ = g_time_format (&timestamp, "%a, %d %b %Y %H:%M:%S %z");
		_tmp82_ = _tmp81_;
		right_label_set_text_or_na (self->priv->unit_active_enter_timestamp_label, _tmp82_);
		_g_free0 (_tmp82_);
	} else {
		right_label_set_text_or_na (self->priv->unit_active_enter_timestamp_label, NULL);
	}
	_tmp83_ = unit_get_active_exit_timestamp (unit);
	t = _tmp83_;
	if (t > 0) {
		struct tm _tmp84_ = {0};
		struct tm _tmp85_ = {0};
		...
		gchar* _tmp86_ = NULL;
		gchar* _tmp87_;
		_tmp85_ = (g_time_local ((time_t) (t / 1000000), &_tmp84_), _tmp84_);
		timestamp = _tmp85_;
		_tmp86_ = g_time_format (&timestamp, "%a, %d %b %Y %H:%M:%S %z");
		_tmp87_ = _tmp86_;
		right_label_set_text_or_na (self->priv->unit_active_exit_timestamp_label, _tmp87_);
		_g_free0 (_tmp87_);
	} else {
		right_label_set_text_or_na (self->priv->unit_active_exit_timestamp_label, NULL);
	}
	_tmp88_ = unit_get_can_start (unit);
	b = _tmp88_;
	...
	right_label_set_text_or_na (self->priv->unit_can_start_label, _tmp89_);
	_tmp90_ = unit_get_can_reload (unit);
	b = _tmp90_;
	...
	right_label_set_text_or_na (self->priv->unit_can_reload_label, _tmp91_);
	_tmp92_ = unit_get_default_control_group (unit);
	_tmp93_ = _tmp92_;
	right_label_set_text_or_na (self->priv->unit_cgroup_label, _tmp93_);
	_g_free0 (_tmp93_);
	_g_free0 (fp);
...
void main_window_update_unit_iter (MainWindow* self, GtkTreeIter* iter, const gchar* id, Unit* u) {
	...
	if (jl.id != 0) {
		...
		gchar* _tmp3_ = NULL;
		gchar* _tmp4_;
		_tmp2_ = dbus_g_proxy_new_for_name (self->priv->bus, "org.freedesktop.systemd1", (const gchar*) jl.path, "org.freedesktop.systemd1.Job");
		j = job_dbus_proxy_new (self->priv->bus, "org.freedesktop.systemd1", (const gchar*) jl.path);
		_tmp3_ = job_get_job_type (j);
		_tmp4_ = _tmp3_;
		_g_free0 (t);
		t = _tmp4_;
		_g_object_unref0 (j);
	...
	_tmp10_ = unit_get_description (u);
	_tmp11_ = _tmp10_;
	_tmp12_ = unit_get_load_state (u);
	_tmp13_ = _tmp12_;
	_tmp14_ = unit_get_active_state (u);
	_tmp15_ = _tmp14_;
	_tmp16_ = unit_get_sub_state (u);
	_tmp17_ = _tmp16_;
	gtk_list_store_set (self->priv->unit_model, iter, 0, id, 1, _tmp11_, 2, _tmp13_, 3, _tmp15_, 4, _tmp17_, 5, _tmp5_, 6, u, -1);
	_g_free0 (_tmp17_);
	_g_free0 (_tmp15_);
	_g_free0 (_tmp13_);
	_g_free0 (_tmp11_);
...
void main_window_on_unit_new (MainWindow* self, const gchar* id, const char* path) {
	...
	_tmp0_ = dbus_g_proxy_new_for_name (self->priv->bus, "org.freedesktop.systemd1", (const gchar*) path, "org.freedesktop.DBus.Properties");
	p = properties_dbus_proxy_new (self->priv->bus, "org.freedesktop.systemd1", (const gchar*) path);
	g_signal_connect_object (p, "properties-changed", (GCallback) _main_window_on_unit_changed_properties_properties_changed, self, 0);
	gtk_list_store_append (self->priv->unit_model, &_tmp1_);
	iter = _tmp1_;
	_tmp2_ = dbus_g_proxy_new_for_name (self->priv->bus, "org.freedesktop.systemd1", (const gchar*) path, "org.freedesktop.systemd1.Unit");
	u = unit_dbus_proxy_new (self->priv->bus, "org.freedesktop.systemd1", (const gchar*) path);
...
void main_window_on_job_new (MainWindow* self, guint32 id, const char* path) {
	...
	p = properties_dbus_proxy_new (self->priv->bus, "org.freedesktop.systemd1", (const gchar*) path);
	g_signal_connect_object (p, "properties-changed", (GCallback) _main_window_on_job_changed_properties_properties_changed, self, 0);
	gtk_list_store_append (self->priv->job_model, &_tmp1_);
	iter = _tmp1_;
	_tmp2_ = dbus_g_proxy_new_for_name (self->priv->bus, "org.freedesktop.systemd1", (const gchar*) path, "org.freedesktop.systemd1.Job");
	j = job_dbus_proxy_new (self->priv->bus, "org.freedesktop.systemd1", (const gchar*) path);
...
void main_window_on_unit_changed (MainWindow* self, Properties* p, const gchar* iface, GHashTable* changed_properties, gchar** invalidated_properties, int invalidated_properties_length1) {
	...
	id = NULL;
	_tmp0_ = dbus_g_proxy_get_bus_name ((DBusGProxy*) p);
	_tmp1_ = dbus_g_proxy_get_path ((DBusGProxy*) p);
	_tmp2_ = dbus_g_proxy_new_for_name (self->priv->bus, _tmp0_, _tmp1_, "org.freedesktop.systemd1.Unit");
	_tmp3_ = dbus_g_proxy_get_bus_name ((DBusGProxy*) p);
	_tmp4_ = dbus_g_proxy_get_path ((DBusGProxy*) p);
	u = unit_dbus_proxy_new (self->priv->bus, _tmp3_, _tmp4_);
	_tmp6_ = gtk_tree_model_get_iter_first ((GtkTreeModel*) self->priv->unit_model, &_tmp5_);
	iter = _tmp5_;
	if (!_tmp6_) {
		_g_object_unref0 (u);
		_g_free0 (id);
	...
	id = _tmp8_;
	{
		...
		while (TRUE) {
			...
			if (!_tmp9_) {
				gboolean _tmp10_;
				_tmp10_ = gtk_tree_model_iter_next ((GtkTreeModel*) self->priv->unit_model, &iter);
				if (!_tmp10_) {
					break;
				}
			}
...
void main_window_on_job_changed (MainWindow* self, Properties* p, const gchar* iface, GHashTable* changed_properties, gchar** invalidated_properties, int invalidated_properties_length1) {
	...
	_tmp0_ = dbus_g_proxy_get_bus_name ((DBusGProxy*) p);
	_tmp1_ = dbus_g_proxy_get_path ((DBusGProxy*) p);
	_tmp2_ = dbus_g_proxy_new_for_name (self->priv->bus, _tmp0_, _tmp1_, "org.freedesktop.systemd1.Job");
	_tmp3_ = dbus_g_proxy_get_bus_name ((DBusGProxy*) p);
	_tmp4_ = dbus_g_proxy_get_path ((DBusGProxy*) p);
	j = job_dbus_proxy_new (self->priv->bus, _tmp3_, _tmp4_);
	_tmp6_ = gtk_tree_model_get_iter_first ((GtkTreeModel*) self->priv->job_model, &_tmp5_);
	iter = _tmp5_;
	if (!_tmp6_) {
		_g_object_unref0 (j);
		return;
	...
	id = _tmp7_;
	{
		...
		while (TRUE) {
			...
			if (!_tmp8_) {
				gboolean _tmp9_;
				_tmp9_ = gtk_tree_model_iter_next ((GtkTreeModel*) self->priv->job_model, &iter);
				if (!_tmp9_) {
					break;
				}
			}
...
void main_window_on_unit_load (MainWindow* self) {
	...
	_tmp3_ = dbus_g_proxy_new_for_name (self->priv->bus, "org.freedesktop.systemd1", (const gchar*) path, "org.freedesktop.systemd1.Unit");
	u = unit_dbus_proxy_new (self->priv->bus, "org.freedesktop.systemd1", (const gchar*) path);
	_tmp4_ = unit_get_id (u);
	_tmp5_ = _tmp4_;
	_tmp6_ = (GtkMessageDialog*) gtk_message_dialog_new ((GtkWindow*) self, GTK_DIALOG_DESTROY_WITH_PARENT, GTK_MESSAGE_INFO, GTK_BUTTONS_CLOSE, "Unit available as id %s", _tmp5_);
	m = (_tmp7_ = g_object_ref_sink (_tmp6_), _g_free0 (_tmp5_), _tmp7_);
...
gboolean main_window_on_activate_link (MainWindow* self, const gchar* uri) {
	...
	_tmp1_ = dbus_g_proxy_new_for_name (self->priv->bus, "org.freedesktop.systemd1", path, "org.freedesktop.systemd1.Unit");
