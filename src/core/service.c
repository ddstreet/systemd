...
static void service_notify_message(
        ...
        if (s->notify_access == NOTIFY_NONE) {
                log_unit_warning(u, "Got notification message from PID "PID_FMT", but reception is disabled.", ucred->pid);
                return;
        } else if (s->notify_access == NOTIFY_MAIN && ucred->pid != s->main_pid) {
                if (s->main_pid != 0)
                        log_unit_warning(u, "Got notification message from PID "PID_FMT", but reception only permitted for main PID "PID_FMT, ucred->pid, s->main_pid);
                else
                        log_unit_debug(u, "Got notification message from PID "PID_FMT", but reception only permitted for main PID which is currently not known", ucred->pid);
                return;
        } else
