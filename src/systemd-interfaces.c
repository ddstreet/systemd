/* systemd-interfaces.c generated by valac 0.11.4, the Vala compiler
...
static void g_cclosure_user_marshal_VOID__UINT_STRING_BOOLEAN (GClosure * closure, GValue * return_value, guint n_param_values, const GValue * param_values, gpointer invocation_hint, gpointer marshal_data);
...
static void _dbus_manager_job_removed (GObject* _sender, guint32 id, const char* path, gboolean success, gpointer* _data);
...
static const GDBusArgInfo _manager_dbus_arg_info_job_removed_success = {-1, "success", "b"};
static const GDBusArgInfo * const _manager_dbus_arg_info_job_removed[] = {&_manager_dbus_arg_info_job_removed_id, &_manager_dbus_arg_info_job_removed_path, &_manager_dbus_arg_info_job_removed_success, NULL};
...
static void g_cclosure_user_marshal_VOID__UINT_STRING_BOOLEAN (GClosure * closure, GValue * return_value, guint n_param_values, const GValue * param_values, gpointer invocation_hint, gpointer marshal_data) {
	typedef void (*GMarshalFunc_VOID__UINT_STRING_BOOLEAN) (gpointer data1, guint32 arg_1, gpointer arg_2, gboolean arg_3, gpointer data2);
	register GMarshalFunc_VOID__UINT_STRING_BOOLEAN callback;
	register GCClosure * cc;
	register gpointer data1, data2;
	cc = (GCClosure *) closure;
	...
	callback = (GMarshalFunc_VOID__UINT_STRING_BOOLEAN) (marshal_data ? marshal_data : cc->callback);
	callback (data1, g_value_get_uint (param_values + 1), g_value_get_string (param_values + 2), g_value_get_boolean (param_values + 3), data2);
}
...
static void manager_base_init (ManagerIface * iface) {
	...
	if (!initialized) {
		...
		g_signal_new ("job_removed", TYPE_MANAGER, G_SIGNAL_RUN_LAST, 0, NULL, NULL, g_cclosure_user_marshal_VOID__UINT_STRING_BOOLEAN, G_TYPE_NONE, 3, G_TYPE_UINT, G_TYPE_STRING, G_TYPE_BOOLEAN);
	}
}
...
static void _dbus_handle_manager_job_removed (Manager* self, GVariant* parameters) {
	...
	success = g_variant_get_boolean (_tmp8_);
...
static void manager_proxy_g_signal (GDBusProxy* proxy, const gchar* sender_name, const gchar* signal_name, GVariant* parameters) {
	if (strcmp (signal_name, "UnitNew") == 0) {
		_dbus_handle_manager_unit_new (proxy, parameters);
	} else if (strcmp (signal_name, "UnitRemoved") == 0) {
		_dbus_handle_manager_unit_removed (proxy, parameters);
	} else if (strcmp (signal_name, "JobNew") == 0) {
		_dbus_handle_manager_job_new (proxy, parameters);
	} else if (strcmp (signal_name, "JobRemoved") == 0) {
		_dbus_handle_manager_job_removed (proxy, parameters);
	}
}
...
static void properties_proxy_g_signal (GDBusProxy* proxy, const gchar* sender_name, const gchar* signal_name, GVariant* parameters) {
	if (strcmp (signal_name, "PropertiesChanged") == 0) {
		_dbus_handle_properties_properties_changed (proxy, parameters);
	}
}
